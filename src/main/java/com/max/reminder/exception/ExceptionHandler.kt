...
class ExceptionHandler {
    ...
    fun handleBusinessException(exception: Exceptions.BusinessException): ResponseEntity<R> {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(R(exception.errorCode, exception.message))
    }
    ...
    fun handlerException(e: Exception): ResponseEntity<R> {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            .body(R(ExceptionEnum.UNEXPECTED_EXCEPTION.exception("Unexpected error on server", e.message)))
    }
}